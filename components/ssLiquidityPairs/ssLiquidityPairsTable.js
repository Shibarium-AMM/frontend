...
function descendingComparator(a, b, orderBy) {
  ...
  switch (orderBy) {
    case 'apy':
      let apyA = BigNumber(a?.apr)
      let apyB = BigNumber(b?.apr)

      return apyA.gt(apyB) ? -1 : 1
...
export default function EnhancedTable({ pairs }) {
  ...
  const [order, setOrder] = useLocalStorage('LIQUIDITY_PAIRS_TABLE_ORDER', 'desc');
  const [orderBy, setOrderBy] = useLocalStorage('LIQUIDITY_PAIRS_TABLE_ORDER_BY', 'stakedBalance');
  const [rowsPerPage, setRowsPerPage] = useLocalStorage('LIQUIDITY_PAIRS_TABLE_ROWS_PER_PAGE', 10);
  const [page, setPage] = useState(0);

  const localToggles = getLocalToggles()

  const [search, setSearch] = useState('')
  const [toggleActive, setToggleActive] = useLocalStorage("LIQUIDITY_PAIRS_TABLE_TOGGLE_ACTIVE", localToggles.toggleActive);
  const [toggleActiveGauge, setToggleActiveGauge] = useLocalStorage("LIQUIDITY_PAIRS_TABLE_TOGGLE_ACTIVE_GAUGE", localToggles.toggleActiveGauge);
  const [toggleStable, setToggleStable] = useLocalStorage("LIQUIDITY_PAIRS_TABLE_TOGGLE_STABLE", localToggles.toggleStable);
  const [toggleVariable, setToggleVariable] = useLocalStorage("LIQUIDITY_PAIRS_TABLE_TOGGLE_VARIABLE", localToggles.toggleVariable);
                    ...
                    <TableCell className={classes.cell} align='right'>
                      <Grid container spacing={0}>
                        <Grid item lg={10}>
                          <Typography variant='h2' className={classes.textSpaced}>
                            {row && (
                              <span>{formatCurrency(row.apr, 2)}%</span>
                            )}
                            {!(row) && (
                              <Skeleton variant="rect"width={120} height={16} style={{ marginTop: '1px', marginBottom: '1px' }} />
                            )}
